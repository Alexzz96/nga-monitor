...
├─────────────────────────────────────────────────────────┤
│  FastAPI (Python 3.11)                                    │
│  - RESTful API (routes/ 目录拆分)                          │
│  - 后台任务 (APScheduler)                                  │
├─────────────────────────────────────────────────────────┤
│  核心模块                                                  │
│  - monitor.py: 监控调度                                    │
│  - nga_crawler.py: NGA 爬虫 (Playwright + 浏览器池)         │
│  - discord_sender.py: Discord 推送 (限流保护)               │
│  - ai_analyzer.py: AI 分析引擎                             │
├─────────────────────────────────────────────────────────┤
│  优化模块                                                  │
│  - browser_pool.py: 浏览器连接池复用                        │
│  - async_logger.py: 异步日志写入                           │
│  - rate_limiter.py: API 限流保护                           │
│  - exceptions.py: 异常分类                                 │
